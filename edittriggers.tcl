...
###
proc edittriggers.create_slave {} { return [interp create] }
...
set edittriggers_api {
    ...
    proc match_gags line {
        ...
        foreach data $gag_data {
            ...
            if { $nocase } {
                if { [regexp -nocase -- $r $line] } { return 1 }
            } {
                if { [regexp -- $r $line] } { return 1 }
            }
        }
        return 0
    }
    ...
    proc car x { return [lindex $x 0] }
    proc cdr x { return [lrange $x 1 end] }
