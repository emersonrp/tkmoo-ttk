...
proc macmoose.invoke_verb_editor {} {
    ...
    ttk::label $e.info.l1 -text "$msg"

    ttk::label $e.info.la -text " args:"
    ttk::entry $e.info.args -width 15 \
    ...
    ttk::label $e.info.lp -text " perms:"
    ttk::entry $e.info.perms -width 4 \
        -background [colourdb.get pink] \
        -font [fonts.fixedwidth]
        $e.info.perms insert 0 $macmoose_editordb($e:VERB_PERMS_)

    ttk::label $e.info.lo -text " owner: $macmoose_editordb($e:VERB_OWNER_)"
...
proc macmoose.do_prop_info data {
    ...
    ttk::label $e.info.lo -text " owner: $macmoose_editordb($e:PROP_OWNER_)"
...
proc macmoose.add_script_or_property browser {
    ...
    ttk::label $w.s.lperms -text "perms:"
    ttk::entry $w.s.perms \
            -width 4 \
            -background [colourdb.get pink] \
            -font [fonts.fixedwidth]
    ...
    ttk::label $w.p.lperms -text "perms:"
    ttk::entry $w.p.perms \
            -width 4 \
            -background [colourdb.get pink] \
            -font [fonts.fixedwidth]
...
proc macmoose.create_browser {} {
    ...
    ttk::frame $browser.lists

    ttk::frame $browser.lists.v
    ttk::label $browser.lists.v.l -text "Scripts / Verbs"

    ttk::frame $browser.lists.v.verbs
    ...
    ttk::scrollbar $browser.lists.v.verbs.s \
    ...
    ttk::frame $browser.lists.p.props
    ...
    ttk::scrollbar $browser.lists.p.props.s \
...
proc macmoose.help {} {
    ...
    ttk::frame $win.entry
    ttk::label $win.entry.text -text "Enter help query: "
    ttk::entry $win.entry.entry
    ttk::button $win.entry.button -text "Search" -command {macmoose.search $win}
    bind $win.entry.entry <Return> {macmoose.search $win}

    ttk::frame $win.msg
    text $win.msg.text -yscrollcommand "$win.msg.sby set"  -state disabled
    ttk::scrollbar $win.msg.sby -orient vert -command "$win.msg.text yview"
    ttk::button $win.close -text "Close" -command {destroy $win}
...
proc macmoose.mail {} {
    ...
    ttk::frame $win.to
    ttk::label $win.to.text -text "To:" -width 8
    ttk::entry $win.to.entry

    ttk::frame $win.subject
    ttk::label $win.subject.text -text "Subject:" -width 8
    ttk::entry $win.subject.entry

    ttk::frame $win.msg
    ttk::label $win.msg.text -text "Message:"
    text $win.msg.msg
    ttk::scrollbar $win.msg.sby -orient vert

    ttk::frame $win.buttons
    ttk::button $win.buttons.send -text "Send" -command {macmoose.verify_mail_recipients $win [$win.to.entry get]}
    ttk::button $win.buttons.cancel -text "Cancel" -command {destroy $win}

    pack $win.to -anchor w
    pack $win.to.text -side left
    pack $win.to.entry -side right
    pack $win.subject -anchor w
